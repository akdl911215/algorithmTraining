def solution(friends, gifts):
    ...
    for i, x in enumerate(friends[0: len(friends) - 1]):
        for y in friends[i + 1: len(friends)]:
                # A > B 보다 클 경우 A의 값 1 증가
                if (table[x][y] > 0 or table[y][x] > 0) and table[x][y] > table[y][x]:
                        index_table[x] += 1
                # B > A 보다 클 경우 B의 값 1 증가
                elif (table[x][y] > 0 or table[y][x] > 0) and table[y][x] > table[x][y]:
                        index_table[y] += 1
