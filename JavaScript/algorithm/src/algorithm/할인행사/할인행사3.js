function solution(want, number, discount) {
  ...
  let count = 0; // 조건을 충족하는 날짜 카운트
  for (let i = 0; i <= discount.length - 10; i++) {
    let tempMap = { ...wantMap }; // 현재 윈도우의 제품 수량을 추적하는 임시 맵
    console.log("tempMap : ", tempMap);

    for (let j = i; j < i + 10; j++) {
      if (tempMap[discount[j]] !== undefined) {
        tempMap[discount[j]]--;
        if (tempMap[discount[j]] === 0) {
          delete tempMap[discount[j]];
        }
      }
    }
    // 임시 맵이 비어있다면 모든 제품이 충분한 수량으로 포함된 것
