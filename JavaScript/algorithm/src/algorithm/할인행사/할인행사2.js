function solution(want, number, discount) {
  if (want.length > discount.length) return null;

  const length = number.reduce(
  ...
  let count = 0;
  let check = 0;
  for (let i = discountArr.length; i < discount.length; ++i) {
    check += 1;

    const boolArr = new Array(wantLength).fill(false);
    console.log(boolArr);
    for (let j = 0; j < discountArr.length; ++j) {
      if (wantSet.has(discountArr[j])) {
        boolArr[j] = true;
      }

      const checking = boolArr.filter((el) => el === false);
...
첫째 날부터 열흘 간에는 냄비가 할인하지 않기 때문에 첫째 날에는 회원가입을 하지 않습니다. 둘째 날부터 열흘 간에는 바나나를 원하는 만큼 할인구매할 수 없기 때문에 둘째 날에도 회원가입을 하지 않습니다.
...
XYZ 마트에서 할인하는 제품을 나타내는 문자열 배열 discount가 주어졌을 때, 회원등록시 정현이가 원하는 제품을 모두 할인 받을 수 있는 회원등록 날짜의
총 일수를 return 하는 solution 함수를 완성하시오. 가능한 날이 없으면 0을 return 합니다.
